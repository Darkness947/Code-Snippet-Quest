...
class DatabaseHelper(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    companion object {
        ...
        const val COLUMN_QUESTION_TEXT_AR = "question_text_ar"
    ...
    private fun insertInitialQuestions(db: SQLiteDatabase?) {
        // Format: level, snippet, question_en, question_ar, opA, opB, opC, opD, correct, hint
        val questions = listOf(
            arrayOf("1",
                "System.out.println(10 + 20 + \"Java\" + 10 + 20);",
                "What is the output of the code snippet above?",
                "ما هو ناتج مقتطف الكود أعلاه؟",
        ...
        for (q in questions) {
            val values = ContentValues().apply {
                // q: [0]=level [1]=snippet [2]=question_en [3]=question_ar
                //    [4]=opA [5]=opB [6]=opC [7]=opD [8]=correct [9]=hint
                put(COLUMN_LEVEL, q[0].toInt())
                put(COLUMN_SNIPPET, q[1])
                put(COLUMN_QUESTION_TEXT, q[2])
                put(COLUMN_QUESTION_TEXT_AR, q[3])
                put(COLUMN_OPTION_A, q[4])
                put(COLUMN_OPTION_B, q[5])
                put(COLUMN_OPTION_C, q[6])
                put(COLUMN_OPTION_D, q[7])
                put(COLUMN_CORRECT_ANSWER, q[8])
                put(COLUMN_HINT, q[9])
            }
    ...
    fun getQuestionsForLevel(level: Int): List<Question> {
        ...
        while (cursor.moveToNext()) {
            // col: 0=id 1=level 2=snippet 3=question_text 4=question_text_ar
            //      5=opA 6=opB 7=opC 8=opD 9=correct 10=hint
            list.add(Question(
                cursor.getInt(0),
                cursor.getInt(1),
                cursor.getString(2),
                cursor.getString(3),
                cursor.getString(4),  // questionTextAr
                cursor.getString(5),
                cursor.getString(6),
                cursor.getString(7),
                cursor.getString(8),
...
data class Question(
    ...
    val level: Int,
    val snippet: String,
    val text: String,           // English question text
    val textAr: String,         // Arabic question text
    val opA: String,
    val opB: String,
    val opC: String,
    val opD: String,
